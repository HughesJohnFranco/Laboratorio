...
def hay_manos(results: NamedTuple) -> bool:
...
# CAPTURE SAMPLES
def dibujar_keypoints(image, results):
    '''
...
def guardar_frames(frames, output_folder):
...
def get_keypoints(model, sample_path):
    ...
    for img_name in os.listdir(sample_path):
        ...
        results = mediapipe_deteccion(frame, model)
